...
use picturium_libvips::{FromSvgOptions, VipsAccess, VipsFailOn, VipsForeignFlags, VipsImage};
...
fn get_svg_options(request: &PipelineRequest, source_path: &str) -> Result<FromSvgOptions> {
    ...
    Ok(FromSvgOptions {
        ...
        stylesheet: request.parameters.style.as_ref().unwrap_or(&"".to_string()).to_string(),
        high_bitdepth: false,
        flags: VipsForeignFlags::None,
        memory: false,
...
fn resolve_scale(request: &PipelineRequest, dpi: f64, source_path: &str) -> Result<f64> {
    let image = VipsImage::new_from_svg(source_path, Some(FromSvgOptions {
        dpi,
        revalidate: true,
        ..Default::default()
    })).map_err(|e| anyhow!(e))?;
